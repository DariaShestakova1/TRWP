...
rootRouter.get('/users/:id', (req, res) => {
    ...
    connection.query(`SELECT readers.readersId, readers.readersName, books.booksId, books.booksName FROM readers INNER JOIN takenBooksList ON readersId = readesId INNER JOIN books ON takenBooksList.bookId = books.booksId WHERE readers.readersId = ${id} AND takenBooksList.readesId = ${id}`, (err, books) => {
        if (err) throw err
        console.log(books);
